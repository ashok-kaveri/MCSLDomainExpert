---
name: mcsl-knowledge-maintainer
description: Use inside MCSLDomainExpert after a MCSL release card cycle is complete, or when QA asks Codex or Claude to update old, wrong, missing, or outdated project knowledge. Updates approved-card RAG, QA retrospective feedback, durable AGENTS/skill rules, and replaces obsolete knowledge without duplicating stale instructions.
---

# MCSL Knowledge Maintainer

Use this skill after a MCSL release card cycle is complete, or when the user asks to update old, wrong, missing, or outdated project knowledge.
...
- durable local instructions such as `AGENTS.md` and MCSL skill references when a rule has truly changed
...
- "add this learning to our MCSL knowledge"
...
1. `mcsl-trello-operator` fetches the real card/list/comment context.
2. `mcsl-ac-writer-reviewer` generates reviewed US + AC.
3. `mcsl-dashboard-tc-publisher` generates reviewed TCs and publish formats.
4. `mcsl-ai-qa-browser` verifies in Chrome and captures evidence/locator traces.
5. `mcsl-bug` handles bug follow-up if needed.
6. `mcsl-automation-writer` writes automation when approved.
7. `mcsl-signoff-message` prepares/sends the QA sign-off when QA confirms.
8. `mcsl-handoff-docs` generates support/business docs if needed.
9. `mcsl-rag-sync` syncs source repos/docs if StorePep server/client, automation, or wiki changed.
10. `mcsl-knowledge-maintainer` updates approved-card RAG, QA feedback, and durable rules.
...
If the request is about pulling latest code/docs or reindexing source knowledge, use `mcsl-rag-sync` first. This skill is for card-cycle learning and outdated rule maintenance, not routine source repo sync.
...
```bash
PYTHONPATH=. .venv/bin/python skills/mcsl-knowledge-maintainer/scripts/maintain_knowledge.py --card-id "<id>" --card-name "<name>" --release "<release>" --description-file "<file>" --ac-file "<file>" --test-cases-file "<file>"
```
...
- MCSL skill files under `skills/mcsl-*`
...
- a MCSL app navigation route changed
...
- `domain_rule`: MCSL/PluginHive/Shopify behavior that future cards need
...
2. Check automation code through `MCSL_AUTOMATION_REPO_PATH` when relevant.
3. Check local wiki/docs if relevant.
4. Browse official MCSL, Shopify, or PluginHive docs only when local knowledge is missing or likely stale.

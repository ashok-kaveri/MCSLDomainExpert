...
Run source sync/reindex only when source documents changed or when the user asks to refresh ChromaDB from local sources. Prefer `mcsl-rag-sync` for routine source sync.

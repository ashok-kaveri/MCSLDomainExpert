...
```bash
PYTHONPATH=. .venv/bin/python skills/mcsl-slack-operator/scripts/slack_ops.py messages --channel "<channel id>" --limit 20
```

For a thread:

```bash
PYTHONPATH=. .venv/bin/python skills/mcsl-slack-operator/scripts/slack_ops.py thread --channel "<channel id>" --ts "<message ts>"
```

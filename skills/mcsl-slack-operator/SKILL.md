---
name: mcsl-slack-operator
description: Use inside MCSLDomainExpert when the user asks Codex or Claude to work with Slack using project .env credentials: search Slack users, list channels, fetch messages from any visible channel, read a thread, send a channel message, reply in a Slack thread, send a DM to a user by name or ID, or coordinate with Trello developer assignment. Requires explicit user intent before any Slack send.
---

# MCSL Slack Operator

Use this skill for Slack read/send/reply tasks in the MCSL QA pipeline.
...
For QA release sign-off messages, prefer `mcsl-signoff-message` because it fetches the Trello line/cards and asks for Backlog links before sending.
...
```bash
PYTHONPATH=. .venv/bin/python skills/mcsl-slack-operator/scripts/slack_ops.py users --query "Ashok"
PYTHONPATH=. .venv/bin/python skills/mcsl-slack-operator/scripts/slack_ops.py channels
PYTHONPATH=. .venv/bin/python skills/mcsl-slack-operator/scripts/slack_ops.py messages --channel "C0123456789" --limit 20
PYTHONPATH=. .venv/bin/python skills/mcsl-slack-operator/scripts/slack_ops.py thread --channel "C0123456789" --ts "1710000000.000000"
PYTHONPATH=. .venv/bin/python skills/mcsl-slack-operator/scripts/slack_ops.py send-dm --user "U0123456789" --text "..."
PYTHONPATH=. .venv/bin/python skills/mcsl-slack-operator/scripts/slack_ops.py send-dm-by-name --query "Dev Name" --text "..."
PYTHONPATH=. .venv/bin/python skills/mcsl-slack-operator/scripts/slack_ops.py send-channel --channel "C0123456789" --text "..."
PYTHONPATH=. .venv/bin/python skills/mcsl-slack-operator/scripts/slack_ops.py reply --channel "C0123456789" --thread-ts "1710000000.000000" --text "..."
```
...
1. Use `mcsl-trello-operator` to get card devs.

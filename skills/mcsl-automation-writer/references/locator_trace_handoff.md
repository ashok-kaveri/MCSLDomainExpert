...
```json
{
  ...
  "source": "mcsl-ai-qa-browser | manual-chrome-inspection | dashboard-ui-trace",
  "tc_ids": ["TC-1"],
  "route": "ORDERS -> Order Summary -> Label Summary -> View Log",
  "page_context": {
    "shopify_admin": true,
    "app_iframe": true,
    "url": ""
...
`mcsl-ai-qa-browser` should save a trace after a successful or partially successful browser run when:

- the user may generate automation later
- a new locator/flow was discovered
- the run involved settings, documents, request logs, product admin, order grid, or app iframe setup

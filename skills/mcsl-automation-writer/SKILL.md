---
name: mcsl-automation-writer
description: Use when working inside the MCSLDomainExpert project after US/AC generation, dashboard TC generation, and AI QA browser verification are complete, and the user wants Playwright TypeScript automation written for an MCSL card. Reuse dashboard automation_writer/chrome_agent conventions, inspect Chrome manually for DOM/locators, reuse existing automation locators/POM methods first, create new locators only when missing, and use saved AI QA locator traces by card id/name when available. The current automation repo uses the Shopify/MCSL flow; explicit non-Shopify cards need QA confirmation first.
---
...
1. `mcsl-ac-writer-reviewer` generates reviewed US + AC.
2. `mcsl-dashboard-tc-publisher` generates dashboard TCs and publish formats.
3. `mcsl-ai-qa-browser` verifies selected TCs in the browser and captures evidence/locators.
4. `mcsl-automation-writer` creates or updates Playwright automation in the automation repo.
5. `mcsl-knowledge-maintainer` records durable automation/QA learning after the cycle is approved.

This skill mirrors the dashboard `Generate Automation Script` behavior while using Codex/Claude app tools.

Current execution surface: Shopify/MCSL automation repo.

For cards explicitly scoped to WooCommerce, BigCommerce, Magento, or PrestaShop, ask QA whether the shared MCSL behavior should be covered through the current Shopify/MCSL automation flow. Generate automation only after QA confirms, and keep the reported platform visible in comments/evidence.
...
If AI QA has not run yet, ask to run `mcsl-ai-qa-browser` first or manually inspect Chrome before writing automation.

## Core Rules

- Confirm the card/test-case platform before writing code. If it is non-Shopify, ask QA whether to cover the shared behavior through the current Shopify/MCSL automation flow before generating code.
...
4. Search automation repo for matching specs/page objects:
   - `rg "<feature keyword>" $MCSL_AUTOMATION_REPO_PATH`
...
- Do not write into the MCSLDomainExpert repo when the task is automation code; write into `MCSL_AUTOMATION_REPO_PATH`.

---
name: mcsl-trello-operator
description: Use inside MCSLDomainExpert when the user asks Codex or Claude to work with Trello using project .env credentials: read boards, lists, cards, descriptions, comments, checklists, attachments, fetch all cards from a list, identify the developer assigned to a card, add comments or QA replies, move cards, search cards, or create generic Trello cards. For QA bug Backlog creation, use mcsl-bug. Requires explicit user intent before any Trello write.
---

# MCSL Trello Operator

Use this skill for Trello actions in the MCSL QA pipeline.
...
For QA bug cards in Backlog, use `mcsl-bug` instead of this operator.
...
```bash
PYTHONPATH=. .venv/bin/python skills/mcsl-trello-operator/scripts/trello_ops.py card --card "<card id or url>"
PYTHONPATH=. .venv/bin/python skills/mcsl-trello-operator/scripts/trello_ops.py comments --card "<card id or url>"
PYTHONPATH=. .venv/bin/python skills/mcsl-trello-operator/scripts/trello_ops.py cards-in-list --list "Dev Done"
PYTHONPATH=. .venv/bin/python skills/mcsl-trello-operator/scripts/trello_ops.py devs --card "<card id or url>"
PYTHONPATH=. .venv/bin/python skills/mcsl-trello-operator/scripts/trello_ops.py add-comment --card "<card id or url>" --text "..."
```
...
1. Use this skill to identify Trello dev members:
   ```bash
   PYTHONPATH=. .venv/bin/python skills/mcsl-trello-operator/scripts/trello_ops.py devs --card "<card>"
   ```
2. Use `mcsl-slack-operator` to search Slack by the dev full name.
...
For final QA sign-off preparation, hand off to `mcsl-signoff-message` after fetching the line/list cards.

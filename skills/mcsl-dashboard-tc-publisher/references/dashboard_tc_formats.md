...
```csv
Sl no,Epics ( Title),Scenario,Feature,Description,Comments,Automated,Details/Transaction ID [Shopify],Pass/Fail [Shopify],Details/Transaction ID [Woocommerce],Pass/Fail [Woocommerce],Remarks
```
...
- Shopify transaction/status columns: fill only for Shopify evidence.
...
- `Settings > account settings `: account setting, account setup, API key, meter number, credentials, MCSL account
